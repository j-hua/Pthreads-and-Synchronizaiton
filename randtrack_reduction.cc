...
void* thread_routine(void *arg){
	...
	for (i = seed_stream_begin; i < seed_stream_end; i++) {
		...
		for (j = 0; j < SAMPLES_TO_COLLECT; j++) {
			...
			if (!(s = hs[thread_index].lookup(key))) {

				// insert a new element for it into the hash table
				s = new sample(key);
				hs[thread_index].insert(s);
			}
...
//		thread_routine((void *)(thread_index+i));
	}
	...
	for (i = 0; i < num_threads; i++) {
		unsigned size = hs[i].get_size();
		unsigned j;
		for (j=0;j<size;j++){
			list<sample,unsigned> entry = hs[i].getList(j);
			unsigned num_ele = entry.num_ele();
			int k;
			for (k=0; k<num_ele; k++){
				sample* s = entry.pop();
				if (!(h.lookup(s->key()))){
